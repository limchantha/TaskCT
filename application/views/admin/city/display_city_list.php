          ...
          <div class="widget-content nopadding">
			<table id="dynamic-table" class="table table-bordered data-table">
													...
													<tbody>
														...
														<tr>															
															<td><?php echo $i;?></td>
															<td><?php echo $task_list->city_name;?></td>
															<td><a class="btn btn-<?php echo $task_list->status=='Active'?'success':'danger';?>" href="<?php echo base_url();?>admin/city/change_status/<?php echo $task_list->id;?>/<?php echo $task_list->status=='Active'?0:1;?>">
																<?php echo $task_list->status;?></a></td>
															<td>															
															<div class="hidden-sm hidden-xs action-buttons">
																<?php if($prev==1 || (!empty($City) && in_array('2',$City))){?>					
																<a class="btn btn-success" href="<?php echo base_url();?>admin/city/add_city/<?php echo $task_list->id;?>">
																	Edit
																</a> 
																<?php } ?>	
																<?php if($prev==1 || (!empty($City) && in_array('3',$City))){?>
																
																<a onclick="return confirm('Once deleted cant be recover again...');" class="btn btn-danger" href="<?php echo base_url();?>admin/city/delete_city/<?php echo $task_list->id;?>">
																	Delete
																</a>
																<?php } ?>
															</div>													
