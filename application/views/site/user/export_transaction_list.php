...
$title = '<table>
	<tr>
        ...
        <td colspan="3" align="left">'.$this->config->item('site_title').' Transaction Details</td>
...
$field_count=count($field_count);
 ...
 foreach ($users_detail as $field_name=>$field_values)
		...
		{
			$data .= '<td style="text-align:left">'.$currency_symbol.' '.($currency_rate*$field_value).'</td>';
		}
		else{
...
header("Content-Disposition: attachment; filename=service_rabbit_transaction.xls");
