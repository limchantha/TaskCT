...
class Stripe {
	 ...
	 */
	public function charge_customer( $amount, $customer_id, $desc ,$currency_type) {
		$params = array(
			'amount' => $amount,
	...
	public function transfer_card( $amount, $card, $desc,$ctype ) {
		$params = array(
			'amount' => $amount,
			'currency' => $ctype,
			'destination' => $card,
	 ...
	 */
	public function card_token_create( $card_data, $amount,$currency_type ) {
		$params = array(
			'card' => $card_data,
			'amount' => $amount,
			'currency' => $currency_type
	...
	private function _send_request( $url_segs, $params = array(), $http_method = 'get' ) {
		if( $this->_conf['stripe_test_mode'] )
			$key = $this->_conf['stripe_key_test_public'];
		else
			$key = $this->_conf['stripe_key_live_public'];
