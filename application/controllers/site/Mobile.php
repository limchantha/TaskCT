...
class Mobile extends MY_Controller {
	...
	{   
        ...
        if($t<=0)
		{
				...
				$headers = apache_request_headers();  
				$save_array=array('first_name'=>$_POST['first_name'],'last_name'=>$_POST['last_name'],'email'=>$_POST['email'],'password'=>$_POST['password'],'phone'=>$_POST['phone'],'device_type'=>$headers['Devicetype'],'device_id'=>$headers['Deviceid'],'zipcode'=>$_POST['zipcode']);
			    ...
			    $userdata = array (
						...
						'phone' => $checkUser->row ()->phone,
						'zipcode' => $checkUser->row ()->zipcode
	...
	public function user_login_process() {
		...
		if ($checkUser->num_rows () == '1') {
			   if($checkUser->row()->photo!='')
				{
					$pro_pic=base_url().'images/site/profile/'.$checkUser->row()->photo;
				}
				else
				{
					$pro_pic=base_url().'images/site/profile/big_avatar.png';
                    ...
                    if($tasker_details->num_rows()>0)
					{
						foreach($tasker_details->result() as $tasker_detail)
						{
							...
							foreach($reviews->result() as $rev){
								...
								if($rev->photo!='')
								{
									$rpro_pic=base_url().'images/site/profile/'.$rev->photo;
								}
								else
								{
									$rpro_pic=base_url().'images/site/profile/big_avatar.png';
								}
								
								$review_array[]=array('user_id'=>$rev->user_id,'pro_pic'=>$rpro_pic,'first_name'=>$rev->first_name,'last_name'=>$rev->last_name,'review_star'=>$rev->rate_val,'review_message'=>$rev->comments,'created'=>date('d-m-Y',strtotime($rev->created)),'date'=>date('M d',strtotime($rev->created)));
							}
							 if($avg_rat!=0)
							 {
	...
	{   
        ...
        if($t<=0)
		{
				$headers = apache_request_headers(); 
				$_POST['password']=md5($_POST['password']); 
				$save_array=array('first_name'=>$_POST['first_name'],'last_name'=>$_POST['last_name'],'email'=>$_POST['email'],'password'=>$_POST['password'],'phone'=>$_POST['phone'],'zipcode'=>$_POST['zipcode'],'device_type'=>$headers['Devicetype'],'device_id'=>$headers['Deviceid'],'group'=>1);
				$t=$this->mobile_model->simple_insert(USERS,$save_array);
	...
	{   
        ...
        if($t->num_rows()==1)
		{
			...
			if($city_list->num_rows()>0)
			{
				...
				{   
				    if($t->row()->work_city==$city->id){
					$metro_city_array[]=array('city_id'=>$city->id,'city_name'=>$city->city_name,'checked_status'=>1);
					}
					else
					{	
					$metro_city_array[]=array('city_id'=>$city->id,'city_name'=>$city->city_name,'checked_status'=>0);
					}
				}
			}
			...
			if($vehicle_list->num_rows()>0)
			{
				...
				{
					if(in_array($vehicle->id,$vehicle_types)){
					$vehicle_list_array[]=array('vehicle_id'=>$vehicle->id,'vehicle_name'=>$vehicle->vehicle_name,'checked_status'=>1);
					}
					else{
					$vehicle_list_array[]=array('vehicle_id'=>$vehicle->id,'vehicle_name'=>$vehicle->vehicle_name,'checked_status'=>0);
					}
				}
			}
			
			$t1['vehicle_list'] =$vehicle_list_array;
			$km_list =array(array('value'=>'50','text'=>"50 Kms"),array('value'=>'100','text'=>"100 Kms"),array('value'=>'150','text'=>"150 Kms"),array('value'=>'200','text'=>"200 Kms"));
			foreach($km_list as $km)
			{   
				if($km["value"]==$t->row()->distance){
					$kmls[]=array('value'=>$km["value"],'text'=>$km["text"],'checked_status'=>1);
				}
				else{
					$kmls[]=array('value'=>$km["value"],'text'=>$km["text"],'checked_status'=>0);
				}
			}
			$tasker_about_us=array('home'=>$t->row()->home,'street'=>$t->row()->street,'city'=>$t->row()->city,'state'=>$t->row()->state,'zipcode'=>$t->row()->zipcode,'dob'=>$t->row()->dob,'distance'=>$t->row()->distance,'detail1'=>$t->row()->detail1,'detail2'=>$t->row()->detail2,'detail3'=>$t->row()->detail3,'hear_about'=>$t->row()->hear_about);
			$t1["km_list"]=$kmls;
	...
	{   
		...
		{      $get_main_category = $this->user_model->get_all_details ( TASKER_CATEGORY, array ('status' => 'Active'));
						...
						$subcat_array=array();
							if($subcat_list->num_rows()>0)
							{
								foreach($subcat_list->result() as $scat){
									...
									if($scat->task_sub_category!=""){
									$cat_array=explode(',',$scat->task_sub_category);
									}
									else{
									$cat_array=array();
									}
								    ...
								    $main_cat_list[]=array('id'=>$mcat->id,'main_cat_name'=>$mcat->task_name,'price'=>$scat->price,'currency_code'=>'USD',"currency_symbol"=>'$',"tasker_description"=>trim(preg_replace('/\s+/', ' ', $scat->tasker_description)),'subcat_id'=>explode(',',$scat->subcat_id),'experience'=>$scat->experience,'supplies'=>$cat_array);
								...
								$main_cat_list[]=array('id'=>$mcat->id,'main_cat_name'=>$mcat->task_name,'price'=>"",'currency_code'=>'USD',"currency_symbol"=>'$',"tasker_description"=>"");	
							}								
			...
			$cleaning_list =array(array('id'=>'1','title'=>'Pickup Truck','description'=>'glass & multi-purpose cleaner sponges, scrub brushes, bucket, rags'),array('id'=>'2','title'=>'Mop','description'=>'with bucket & floor cleaner'),array('id'=>'3','title'=>'Vacuum','description'=>'Vacuum cleaner with cleaning tools'));	
			foreach($cleaning_list as $clean)
			{
				if(in_array($clean['id'],$cat_array)){
					$cleanls[]=array("id"=>$clean['id'],"title"=>$clean["title"],"description"=>$clean["description"],'checked_status'=>"1");
				}
				else{
				$cleanls[]=array("id"=>$clean['id'],"title"=>$clean["title"],"description"=>$clean["description"],'checked_status'=>"0");
				}
			}
			$t1['cleaning_list'] =$cleanls;
			$t1['status'] =1;
		}
		else
		{
		...
		{      
				 ...
				 $sub_cat_list=array();
				  $catid=$_POST['task_category_id'];
						...
						$cat_list=$this->user_model->get_all_details ( TASKER_CATEGORY_SELECTION, array ('task_category_id'=>$catid,'user_id'=>$t->row()->id));
						if($cat_list->num_rows()==1){
							$cat_array_exist=explode(',',$cat_list->row()->subcat_id);
							$exp=$cat_list->row()->experience;
							$des=trim(preg_replace('/\s+/', ' ', $cat_list->row()->tasker_description));
						}
						else
						{
							$cat_array_exist=array();
							$exp="";
							$des="";
						}
						...
						$subcat_array=array(); 
							if($subcat_list->num_rows()>0)
							{
								foreach($subcat_list->result() as $scat){
									...
									if(in_array($scat->id,$cat_array_exist)){
									$subcat_array[]=array('id'=>$scat->id,'sub_cat_name'=>$scat->subcat_name,'checked_status'=>"1");
									}
									else{
									$subcat_array[]=array('id'=>$scat->id,'sub_cat_name'=>$scat->subcat_name,'checked_status'=>"0");
									}
								}
								#$t1['experience']=$exp;
								#$t1["tasker_description"]=$des;
	...
	{   
		...
		{      
				 ...
				 $sub_cat_list=array();
				  $catid=$_POST['task_category_id'];
					    $experience_list=array(array("value"=>"none","text"=>"No experience, but I am willing to learn"),array("value"=>"some","text"=>"Some experience. I have done it for myself around the home"),array("value"=>"part_time","text"=>"I have had part-time experience"),array("value"=>"professional","text"=>"I have had professional experience"),array("value"=>"certified","text"=>"I am professionally certified in this skill"));
						$cat_list=$this->user_model->get_all_details ( TASKER_CATEGORY_SELECTION, array ('task_category_id'=>$catid,'user_id'=>$t->row()->id));
						if($cat_list->num_rows()==1){
							
							foreach($experience_list as $exlist){
								if($exlist["value"]==$cat_list->row()->experience){
								$exp_array[]=array("value"=>$exlist["value"],"text"=>$exlist["text"],"checked_status"=>"1");
								}
								else{
								$exp_array[]=array("value"=>$exlist["value"],"text"=>$exlist["text"],"checked_status"=>"0");
								}
							}
						}
						else
	...
	{   
		...
		{      
				...
				$st=$userdetail->stripe_user_id==""?"Connect to Stripe":"Reconnect Stripe";
				$stripe_pay=(json_decode($this->config->item('stripe_payment')));
			    $stripe_connect_url="https://connect.stripe.com/oauth/authorize?response_type=code&scope=read_write&client_id=".$stripe_pay->client_id;
				$_SESSION["stripe_user_id_webview"]=$userdetail->id;
				redirect($stripe_connect_url);
	...
	{   
		$email=$_POST['email'];
		$t=$this->mobile_model->get_all_details(USERS,array('email'=>$email,'group'=>0)); 
		if($t->num_rows()==1)
		{     
    ...
    public function tasker_forgot_password()
	{   
		$email=$_POST['email'];
		$t=$this->mobile_model->get_all_details(USERS,array('email'=>$email,'group'=>1)); 
		if($t->num_rows()==1)
		{     
	...
	public function tasker_login_process() {
		...
		if ($checkUser->num_rows () == '1') {
				...
				$datestring = "%Y-%m-%d %h:%i:%s";
				$time = time ();
				$headers = apache_request_headers();  #print_r($headers); echo $headers['Deviceid'];
				if($headers['Devicetype']!=""){
				$newdata = array (
						'last_login_date' => mdate ( $datestring, $time ),
						'last_login_ip' => $this->input->ip_address (),
						'device_type'=>$headers['Devicetype'],
	...
	{   
		...
		{       $message_array=array();
				...
				if($t->num_rows()>0)
				{
					$t1['unreadmessage_count'] =$t->num_rows();
				}
				else
				{
				...
				if($message_list->num_rows()>0){
					foreach($message_list->result() as $mess){ #echo '<pre>'; print_r($mess);
						...
						$message_array[]=array('name'=>$mess->first_name,'profile_image'=>$pro_pic,'task_name'=>$mess->task_name,'created_time'=>date('h:i a',strtotime($mess->msg_time)),'message'=>$mess->msg,'booking_id'=>$mess->booking_id,'message_status'=>$mess->message_status,'booking_status'=>$mess->bstatus);
					}
					$t1['status']=1;
	...
	{   
		...
		{       $message_array=array();
				$id=$t->row()->id;
				$booking_id=$_POST['booking_id'];
			    $ptask=$this->mobile_model->get_completed_task_info($booking_id)->row(); #echo $this->db->last_query();
				...
				$supply_array=array(); #echo '<pre>';print_r($ptask);
				if($ptask->task_sub_category!='')
				{
					if(in_array(1,explode(',',$ptask->task_sub_category)))
					{
						$supply_array[]=array('title'=>'Basic Supplies','description'=>'glass & multi-purpose cleaner sponges, scrub brushes, bucket, rags');
					}
					if(in_array(2,explode(',',$ptask->task_sub_category)))
					{
						$supply_array[]=array('title'=>'Mop','description'=>'with bucket & floor cleaner');
					}
					if(in_array(3,explode(',',$ptask->task_sub_category)))
					{
						$supply_array[]=array('title'=>'Vacuum','description'=>'');
					}
				}
	...
	{   
		#$this->push_notification_user_ios('5A7DA5CE83720E9127F3E31284C4E9512C972C51DEFCA82F72A8D67C51B7829C','TEST user by siva');
		#$this->push_notification_tasker_ios('88B2074D57CE887B6AA5CEB5D9F32EBB44462A65C616FA6D4B7861C4619E0DCC','TEST tasker by siva');
		$this->load->library('Apns');
		$this->apns->send_push_message_user_new('C5B1DACB73772D59C454D13264DBBC9FDE2ECF99A101217E6CC0210440A30DF3','TEST user by siva',"user_request"); 
		echo "Success";
	}
	...
	public function tasker_transaction_list()
	{
		    $perpage=50;
			$page=1;
		    $email=$_POST['email'];	
			...
			$usertrans=$this->mobile_model->export_transaction_list_tasker($id,$pageLimitStart,$perpage); 
			$transaction_list=array();
			if($usertrans->num_rows()>0){
			foreach($usertrans->result() as $trans)
			{
				...
				$transaction_list[]=array('user_name'=>$trans->Tasker_name,'task_name'=>$trans->Task_name,'address'=>$trans->address,'pro_pic'=>$pro_pic,'booking_date'=>$trans->booking_date,'booking_day'=>date('d',strtotime($trans->booking_date)),'booking_month'=>date('M',strtotime($trans->booking_date)),'booking_time'=>$btime,'vehicle'=>$trans->veh_name,'currency_symbol'=>'$','paid_amount'=>$trans->Paid_amount);
			}
			$ret['status']=1;
		    $ret['export_result']=$transaction_list;
			}
	...
	public function update_message_status()
	{
			...
			$ret['status']=1;
		    $ret['message']='Message update done';
			}
			else
			{
			$ret['status']=0;	
	...
	public function check_tasker_available()
	{
		    ...
		    if($checkUser->num_rows()==1){			
				...
				$task_id = $this->input->post('task_category_id');
				...
				$json = json_decode($json);
					if($json->status=='OK' && isset($json->{'results'}[0]->{'geometry'}->{'bounds'}))
					{
						...
						{
							$ret['status']=1;
						    $ret['message']='Taskers avilable';
						}
						else 
						{
							$ret['status']=0;
						    $ret['message']='Taskers Not Available with this city';
						}
					...
					{
					$ret['status']=0;	
					$ret['message']='Sorry, No Tasker available';
					}
				}
				else
				{
	...
	{   
		...
		{       $message_array=array();
				$id=$t->row()->id;
				$booking_id=$_POST['booking_id'];
			    $message_list=$this->mobile_model->get_retrival_messages($booking_id);
				...
				if($message_list->num_rows()>0){
					foreach($message_list->result() as $mess){ #echo '<pre>'; print_r($mess);
						...
						if($mess->uphoto!='')
						{
							$pro_pic=base_url().'images/site/profile/'.$t->row()->photo;
						}
						else
						{
							$pro_pic=base_url().'images/site/profile/big_avatar.png';
						...
						if($mess->uphoto!='')
						{
							$pro_pic=base_url().'images/site/profile/'.$mess->uphoto;
						}
						else
						{
							$pro_pic=base_url().'images/site/profile/big_avatar.png';
						...
						$message_array[]=array('message'=>$mess->message,'position'=>$position,'booking_id'=>$mess->booking_id,'pro_pic'=>$pro_pic,'created_time'=>date('h:i a',strtotime($mess->created)));
					}
					$t1['status']=1;
					$getbooking_user=$this->mobile_model->getbookinfo($booking_id)->row();
					if($id==$getbooking_user->uid){
					     if($getbooking_user->tphoto!='')
						{
							$pro_pic=base_url().'images/site/profile/'.$getbooking_user->tphoto;
						}
						else
						{
							$pro_pic=base_url().'images/site/profile/big_avatar.png';
						}	
                        $t1['chat_user_info']=$msg_info=array('name'=>$getbooking_user->tfirst_name,'profile_image'=>$pro_pic,'city'=>$getbooking_user->address);	
					}
					else
					{
					    if($getbooking_user->uphoto!='')
						{
							$pro_pic=base_url().'images/site/profile/'.$getbooking_user->uphoto;
						}
						else
						{
							$pro_pic=base_url().'images/site/profile/big_avatar.png';
						}	
                        $t1['chat_user_info']=$msg_info=array('name'=>$getbooking_user->ufirst_name,'profile_image'=>$pro_pic,'city'=>$getbooking_user->address);
					}						
					$t1['chat_messages']=$message_array;
	...
	public function chat_inner_tasker()
	{   
		#print_r($_POST);die;
		$email=$_POST['email'];
		...
		{       $message_array=array();
				...
				if($ptask->photo!='')
				{
					$pro_pic=base_url().'images/site/profile/'.$ptask->uphoto;
				}
				else
				{
					$pro_pic=base_url().'images/site/profile/big_avatar.png';
				...
				$booking_info=array('booking_id'=>$ptask->booking_id,'tasker_id'=>$ptask->tasker_id,'first_name'=>$ptask->ufirst_name,'last_name'=>$ptask->ulast_name,'pro_pic'=>$pro_pic,'cat_name'=>$ptask->task_name,'subcat_name'=>$ptask->subcat_name,'booking_day'=>date('d',strtotime($ptask->booking_date)),'booking_month'=>date('M',strtotime($ptask->booking_date)),'booking_time'=>$btime,'need_vehicle'=>$ptask->veh_name,'city'=>$ptask->address,'total_amount'=>$ptask->total_amount,'currency_code'=>'USD',"currency_symbol"=>'$','status'=>$ptask->status,'supplies'=>$supply_array);
				$t1['booking_info']=$booking_info;		
				$t1['chat_user_info']=array('name'=>$ptask->ufirst_name,'profile_image'=>$pro_pic,'city'=>$ptask->address);		
	...
	public function get_message_list_tasker()
	{   
		#print_r($_POST);die;
		$email=$_POST['email'];
